...
import aiBrain from "@/assets/ai-brain";
  ...
  return (
    <main id="top" className="overflow-x-hidden">
      ...
      <section id="about" className="relative overflow-hidden bg-background">
        <NeuralNetworkBG tone="light" />

        <div className="relative z-10 mx-auto grid max-w-6xl gap-12 px-6 py-24 sm:px-10 lg:grid-cols-2 lg:items-center lg:gap-16 lg:px-14 lg:py-28">
          <div className="order-2 lg:order-1">
            ...
            <Reveal
              ...
              variant="left"
              delay={60}
              className="mt-4 text-[1.85rem] leading-[1.2] font-light tracking-tight sm:text-[2.15rem] lg:text-[2.35rem]"
            >
            ...
            <Reveal
              ...
              variant="left"
              delay={120}
              className="mt-6 max-w-md text-[15px] leading-relaxed font-light tracking-tight text-foreground/80 sm:text-[1.05rem]"
            >
              Python and SQL as the foundation, strong{" "}
              <span className="text-muted-foreground">DSA</span> fundamentals, and hands-on work
              across machine learning, deep learning, NLP and generative AI — from classical models
            ...
            <Reveal
              ...
              variant="left"
              delay={180}
              className="mt-4 max-w-md text-[14px] leading-relaxed text-muted-foreground"
            >
              I care about clean pipelines, measurable evaluation, and shipping models that teams
              can actually trust in production.
            </Reveal>

            <div className="mt-10 grid grid-cols-3 gap-3 sm:gap-4">
              {[
                { n: "ML · DL", l: "Core stack" },
                { n: "NLP · RAG", l: "Language AI" },
                { n: "GenAI", l: "Agents & LLMs" },
              ].map((s, i) => (
                <Reveal key={s.l} variant="scale" delay={220 + i * 70}>
                  <div className="section-card rounded-2xl border border-border/70 bg-card/75 px-3 py-4 text-center shadow-sm backdrop-blur-sm sm:px-4">
                    <div className="text-sm font-medium tracking-tight text-foreground sm:text-base">
                      {s.n}
                    </div>
          ...
          <Reveal variant="scale" delay={100} className="order-1 lg:order-2">
            <div className="relative mx-auto max-w-md lg:max-w-none">
              <div className="absolute -inset-4 rounded-[2rem] bg-gradient-to-br from-sky-400/15 via-transparent to-amber-400/10 blur-2xl" />
              <img
                src={aiBrain}
                alt="AI brain neural network visualization — biological and digital hemispheres"
                width={720}
                height={720}
                loading="lazy"
                className="relative z-[1] w-full rounded-3xl object-cover shadow-2xl shadow-sky-900/10 ring-1 ring-border/40"
              />
            </div>
          </Reveal>
        </div>
              ...
              return (
                <Reveal key={c.label} variant="scale" delay={i * 90} className="h-full">
                  <Tilt
                    ...
                    max={7}
                    lift={14}
                    className="section-card group flex h-full flex-col gap-4 rounded-2xl border border-border/70 bg-card/80 p-6 transition-colors hover:bg-card"
                  >
                    <span className="flex size-10 items-center justify-center rounded-xl bg-secondary text-foreground transition-colors group-hover:bg-accent group-hover:text-accent-foreground">
                      <Icon />
                    </span>
                    <div className="layer-lift">
                      <span className="text-[10px] tracking-wider text-muted-foreground uppercase">{c.label}</span>
                      <span className="mt-1 block text-[13px] font-medium tracking-tight text-foreground transition-colors group-hover:text-accent">
                        {c.value}
                      </span>
                    </div>
                  </Tilt>
